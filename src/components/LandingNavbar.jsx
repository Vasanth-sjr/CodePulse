...
import { BarChart2, Link, ClipboardList, Brain, Lightbulb, AlertTriangle } from 'lucide-react';
import CodePulseLogo from './CodePulseLogo';

/* ── dropdown data ── */
const platformItems = [
  { icon: <BarChart2 className="inline-block w-4 h-4" />, label: 'Developer Intelligence', desc: 'Impact scoring beyond commit counts' },
  { icon: <ClipboardList className="inline-block w-4 h-4" />, label: 'Plan vs Reality', desc: 'Jira tasks matched to GitHub commits' },
  { icon: <Brain className="inline-block w-4 h-4" />, label: 'Skill Intelligence', desc: 'Expertise mapped from code contributions' },
  { icon: <AlertTriangle className="inline-block w-4 h-4" />, label: 'Risk Detection', desc: 'Identify single points of failure' },
];

const whyItems = [
  { icon: '🎯', label: 'For Engineering Leaders', desc: 'Data-driven team management' },
  { icon: <Lightbulb className="inline-block w-4 h-4" />, label: 'For Developers', desc: 'Fair, transparent contribution scoring' },
];

const resourceItems = [
  { icon: '📖', label: 'Documentation', desc: 'Guides and API reference' },
  { icon: <Link className="inline-block w-4 h-4" />, label: 'GitHub', desc: 'Open source repository' },
];
